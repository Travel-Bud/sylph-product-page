...
**Built.** Scripted kinematics plus fixed-step springs, no randomness (`engine.ts`, `machine.ts`). A scheduler holds charges in single file by reserving doors. Four scenes (policy compiled, the match, same answer, month end) and a close. Reduced motion shows the settled board. Sound goes through `v2-sides/sound.ts` with a nav toggle. gen-image spend: $0.

**To ship.** Real policy rules in place of the six fixed gates. A layout pass at 641 to 980px. An ear check of the sound: gate clicks are booked, but headless capture can't hear them.
