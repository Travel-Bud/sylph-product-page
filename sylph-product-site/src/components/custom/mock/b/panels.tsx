...
import "@/components/custom/v2-sides/chapters.css";
...
/* Mock B: the live landing's chapter panels (v2-sides/chapters.tsx), copied so the chapters can be
   recomposed around them. Changes from the live copy: the compile reads its scroll track from
   [data-pin-track] instead of its section, the verdict picker is rebuilt in chapters.tsx around AnswerDetail, and the
   desk queue drops its small strip (the chapter's wall above it shows the twenty charges).
   Original note: every chapter is one composed screen. The person stands on their own edge, the copy sits
...
export function useArrival(stop: number, fn: () => void) {
  const ref = useRef(fn);
  useEffect(() => {
    ref.current = fn;
  });
...
export function StopPulse({ stop }: { stop: number }) {
  useArrival(stop, () => {
...
export function tabKeys(e: React.KeyboardEvent<HTMLElement>, i: number, n: number, set: (j: number) => void) {
  const k = e.key;
  let j = -1;
...
export function PolicyCompile() {
  ...
  useEffect(() => {
    ...
    const section = el?.closest<HTMLElement>("[data-pin-track]");
...
export function AnswerDetail({ id }: { id: string }) {
  if (id === "sushi")
    return (
...
export function StepLine({ step, state }: { step: number; state: string }) {
  return (
